...
class MenuItemsTableSeederCustom extends Seeder
{
    ...
    public function run()
    {
        ...
        $menuItem = MenuItem::firstOrNew(["menu_id" => $menu->id, "title" => "Currencies", "url" => "", "route" => "voyager.currencies.index", ]);
        if (!$menuItem->exists) {
            $menuItem->fill(["target" => "_self", "icon_class" => "voyager-world", "color" => "", "parent_id" => null, "order" => "17", ])->save();
        }
